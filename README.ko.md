...
flowchart TD
    ...
    subgraph DevPhase["1. Development Phase (개발 및 코딩 환경)"]
        direction TB
        A["make build-ros / build-dev<br/>(개발용 도커 이미지 빌드)"] --> B["make ros / dev<br/>(컨테이너 기동 및 셸 진입)"]
        B --> C["VS Code Attach<br/>(호스트 에디터 연결 및 개발)"]
        C --> D["mksync / cb / mbuild<br/>(의존성 동기화 및 패키지 빌드)"]
    ...
    subgraph BakePhase["2. Packaging Phase (SIF 이미지 생성)"]
        direction TB
        E["make bake / bake-prod<br/>(전체 워크스페이스 상태를 SIF 파일로 변환)"]
    ...
    subgraph DeployPhase["3. Execution Phase (실행 및 운영)"]
        ...
        H["make run-slurm<br/>(HPC 클러스터 스케줄러에 작업 제출)"]
    end
...
| :------------ | :------------------- | :--------------------- | :--------------------- | :------------------- | :------------------ |
| **ROS 환경**  | **`make ros`**       | **`make ros-stop`**    | **`make ros-restart`** | **`make ros-shell`** | **`make ros-term`** |
